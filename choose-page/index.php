...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['Cho_Page']) and $_POST['Cho_Page'] !== 'NewType') {
        ...
        if ($_POST['Page_Name'] !== '') {
            $ForSQL .= 'page_name = "' . $_POST['Page_Name'] . '" ,';
        }
        if ($_POST['PageContent'] !== '') {
            $ForSQL .= 'page_content = "' . $_POST['PageContent'] . '" ,';
        }
    ...
    if ($_POST['Type'] == 'Delete') {
        ...
        $sql = $pdo->prepare($ForSQL);
        $sql->execute([$_POST['PageId']]);
        $OtherPageWord = '';
        foreach ($sql->fetchAll() as $row) {
            $OtherPageWord .= $row['word_id'] . ',';
        }
        ...
        $WordJustInThisPage = '';
        foreach ($sql->fetchAll() as $row) {
            $WordJustInThisPage .= $row['word_id'] . ',';
        }
...
<head>
    ...
    <script>
        var PageListArr = <?php echo $json_array ?>
    </script>
</head>
    ...
    <div id="EditBox">
        ...
        <div id="ButtomFormBox" class="collapse">
            <form action="" method="post">
                ...
                <h3>名稱</h3>
                <input type="text" name="Page_Name" placeholder="頁目名稱">
                <hr>
                <h3>描述</h3>
                <textarea name="PageContent" id="" placeholder='本填入框將用於描述頁目'></textarea>
                <hr>
            ...
            <div class="Special_Check modal-content">
                ...
                <form action="" method="post">
                    <div id="CheckBox">
                        ...
                        <div id="checkIfDel">
                            <p>將會刪除本頁以及本頁獨有的字詞。<br>確定刪除嗎？此操作將不可逆。</p><input type="submit" value="確定">
                        </div>
                        <div id="checkIfCopy">
                            <p>註：字詞編號不受頁目的影響，若想複製字詞請於頁內操作。</p><input type="submit" value="確定">
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.querySelectorAll('.ScrollOutBut').forEach(value => {
            value.addEventListener('click', function(event) {
                document.querySelectorAll('#CheckBox div').forEach(ScrollIn => {
                    ScrollIn.classList.remove('active')
                })
        ...
        document.querySelectorAll('.PaneIcon').forEach(value => {
            value.addEventListener('click', function(event) {
                ...
                if (PageListArr[PageId]['public'] == 1) {
                    publicBut.checked = true;
                } else {
                    publicBut.checked = false;
                }
            })
